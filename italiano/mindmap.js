// NEON ITALIANO — Interactive HTML5 Canvas Mind Map (Carte Mentale 2D Interactive)
// Built with the exact same physics, particles, gesture, zoom and touch engine as NEON ENGLISH.
...
class NeonItalianMindMap {
  ...
  loadWord(wordObj) {
    ...
    // Root node (Center word)
    const centerNode = {
      id: 'center',
      label: String(wordObj.word || ''),
      sub: String(wordObj.translation || ''),
      type: 'root',
      x: cx,
      y: cy,
      vx: 0,
    ...
    const satellites = [
      {
        id: 'hook',
        label: '🧠 Memoria',
        sub: wordObj.hook || 'Ancre mnémotechnique',
        color: '#d8bd78',
        children: []
      },
      {
        id: 'synonyms',
        label: '🔗 Sinonimi',
        sub: synonyms.join(', ') || 'Nessun sinonimo',
        color: '#00e6a0',
        children: synonyms.slice(0, 4).map((s, idx) => ({ id: 'syn_' + idx, label: s, color: '#5bf5c2' }))
      },
      {
        id: 'antonyms',
        label: '⚡ Contrari',
        sub: antonyms.join(', ') || 'Nessun contrario',
        color: '#ff5a5f',
        children: antonyms.slice(0, 3).map((a, idx) => ({ id: 'ant_' + idx, label: a, color: '#ff8a8e' }))
      },
      {
        id: 'collocations',
        label: '💼 Collocazioni',
        sub: collocations.slice(0, 3).join(' • ') || 'Nessuna collocazione',
        color: '#ffb347',
        children: collocations.slice(0, 3).map((c, idx) => ({ id: 'col_' + idx, label: c, color: '#ffd166' }))
      },
      {
        id: 'story',
        label: '🎭 Mini-Storia',
        sub: wordObj.story || wordObj.storyFr || '',
        color: '#d8bd78',
        children: []
      },
      {
        id: 'speak',
        label: '🗣️ Pratica & Audio',
        sub: '“' + String(wordObj.example || wordObj.word || '') + '”',
        color: '#00e6a0',
        children: []
      }
    ];
  ...
  onNodeClick(node) {
    ...
    // Trigger Italian Speech Synthesis if central word or speak node
    if ((node.id === 'speak' || node.type === 'root') && this.activeWord) {
      if (window.speakItalian) {
        window.speakItalian(this.activeWord.example || this.activeWord.word);
      } else if ('speechSynthesis' in window) {
        window.speechSynthesis.cancel();
        const u = new SpeechSynthesisUtterance(this.activeWord.example || this.activeWord.word);
        u.lang = 'it-IT';
        u.rate = 0.88;
    ...
    if (infoBox) {
      ...
      infoBox.innerHTML = `<b style="color:${node.color}">${label}</b><div style="color:var(--muted,#aab4b0);margin-top:4px;">${sub}</div>`;
    }
  }
  ...
  draw() {
    ...
    this.nodes.forEach(n => {
      ...
      ctx.strokeStyle = isRoot ? '#06100d' : '#f5f7f6';
      ctx.lineWidth = isHovered ? 2.5 : 1.2;
      ctx.stroke();

      // Text label
      ctx.fillStyle = isRoot ? '#06100d' : '#f5f7f6';
